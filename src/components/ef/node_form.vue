      ...
      <div class="ef-node-form-body">
        <el-form :model="node" ref="dataForm" label-width="80px" v-show="type === 'node'">
          ...
          <el-form-item label="背景色">
            <el-input v-model="node.background"></el-input>
          </el-form-item>
          ...
          <el-form-item label="标签">
            <el-select
              ...
              placeholder="请选择标签">
              <el-option v-for="(tag, i) in node.tags" :key="`${tag}-${i}`" :label="tag" :value="tag"></el-option>
            </el-select>
          </el-form-item>
    ...
    save () {
      this.data.nodeList.filter((node) => {
        if (node.id === this.node.id) {
          ...
          node.background = this.node.background
          node.remark = this.node.remark
          node.tags = this.node.tags
